//! `blurt` — drive the real pipeline from a terminal.
//!
...
use blurt_core::{AppContext, DictationEngine, Mode, ProviderCredentials, Vocabulary, SAMPLE_RATE};
use clap::{Parser, Subcommand};

#[derive(Parser)]
#[command(name = "blurt", version, about = "Blurt pipeline CLI")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn credentials_from_env() -> HashMap<String, ProviderCredentials> {
    ...
    for (id, key_var, url_var) in [
        ...
        ("openai-compat", "BLURT_API_KEY", "BLURT_BASE_URL"),
    ] {
...
fn explain_missing_credentials(err: &blurt_core::DictError, dotenv_path: Option<&std::path::Path>) {
    let blurt_core::DictError::NotConfigured { provider, .. } = err else {
        return;
    };
    let var = match provider.as_str() {
        ...
        _ => "BLURT_API_KEY (with BLURT_BASE_URL)",
    };
    match dotenv_path {
        ...
        Command::Providers => {
            ...
            for id in ["groq", "openai", "deepgram", "ollama", "lmstudio", "vllm"] {
                println!(
                    "{:<16} {}",
                    id,
                    blurt_core::providers::default_base_url(id).unwrap_or("-")
                );
            }
            println!(
                "\nAny OpenAI-compatible endpoint also works via --stt-provider openai-compat"
            );
            println!("with BLURT_BASE_URL / BLURT_API_KEY set.");
        ...
        Command::Transcribe {
            ...
            match llm_provider.as_deref() {
                ...
                Some(p) => {
                    ...
                    m.cleanup = Some(blurt_core::LlmConfig {
